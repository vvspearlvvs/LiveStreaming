...
import com.example.wearable.camera6.exceptions.StorageUnavailableException;
...
public class Session {
	...
	public Session() {
		...
		HandlerThread thread = new HandlerThread("Session Thread");
		thread.start();
		Log.i(TAG,"SESSION 스레드 시작");
	...
	public String getSessionDescription() {
		...
		if (mAudioStream != null) { //Body.createAudioBody(trackAudio, sampleRate, isStereo)
			sessionDescription.append(mAudioStream.getSessionDescription());
			sessionDescription.append("a=control:trackID="+0+"\r\n");
		}
		if (mVideoStream != null) { //Body.createVideoBody(trackVideo, sSPS, sPPS)
	...
	private void syncStop(final int id) {
		Stream stream = (Stream) (id==0 ? mAudioStream : mVideoStream);
		if (stream!=null) {
			stream.stop();
		}
	}		
