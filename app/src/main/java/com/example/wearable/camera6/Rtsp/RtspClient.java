...
package com.example.wearable.camera6.Rtsp;
...
import com.example.wearable.camera6.Rtp.RtpSocket;
...
/**
	...
	private class Parameters {
		...
		public Parameters clone() {
			Parameters params = new Parameters();//X
			params.host = host;
			params.path = path;
	...
	private Parameters mTmpParameters;//X
	private Parameters mParameters;//X

	private int mCSeq;
	private Socket mSocket;
	private String mSessionID;//X
	private String mAuthorization;//X
	private BufferedReader mBufferedReader;
	private OutputStream mOutputStream;
	private Callback mCallback;//X
	private Handler mMainHandler;//X
...
/**
	...
	public RtspClient() {
		...
		new HandlerThread("RtspClient Thread") {
			@Override
	...
	public void startStream() {
		...
		mHandler.post(new Runnable() {
			...
			public void run() {
				...
				Log.i(TAG,"호스트"+mTmpParameters.host+"포트"+mTmpParameters.port+mTmpParameters.path+mTmpParameters.session+mTmpParameters.transport);
...
/**
	...
	private void abort() {
		...
		try {
			...
			Log.i(TAG,"왜연결못하고TEARDOWN"+ignore);
		}
	...
	private void tryConnection() throws IOException {
		...
		Log.i(TAG,"소켓연결"+mSocket);
		mBufferedReader = new BufferedReader(new InputStreamReader(mSocket.getInputStream()));
		mOutputStream = new BufferedOutputStream(mSocket.getOutputStream());
		//setCredentials("user	","wearable");
		sendRequestAnnounce();
		Log.i(TAG,"Client announce끝");
		sendRequestSetup();
		Log.i(TAG,"Client setup끝");
		sendRequestRecord();
		Log.i(TAG,"Client record끝");
...
/**
	...
	private void sendRequestAnnounce() throws IllegalStateException, SocketException, IOException {
		...
		Log.i(TAG,"들어온 응답헤더 사이즈"+response.headers.size());
		...
		if (response.status == 401) {
			...
			String hash3 = computeMd5Hash(m.group(2) + ":" + hash2);

		*/
/*	mAuthorization =  "이건무엇"+hash1+"realm=\"" + realm + "\",nonce=\"" + nonce + "\",uri=\"" + uri + "\",response=\"" + hash3 + "\"";
			Log.i(TAG,"허가인증"+mAuthorization);
*//*
		...
		public void run() {
			if (mState == STATE_STARTED) {
				try {
					...
					Log.i(TAG, "Connection lost with the server...");
					mParameters.session.stop();
...
/**
		...
		public static Response parseResponse(BufferedReader input) throws IOException, IllegalStateException, SocketException {
			...
			while(line!=null){
				...
				if(line == null) throw new SocketException("Connection lost"+response.status);
				matcher = regexStatus.matcher(line);
				matcher.find();
				response.status = Integer.parseInt(matcher.group(1));
				Log.i(TAG,"응답상태코드"+response.status);
				if (line.length() > 3) {
					matcher = rexegHeader.matcher(line);
					matcher.find();
					Log.i(TAG,"응답온거"+matcher);
			...
			if ((line = input.readLine()) == null) throw new SocketException("Connection lost"+response.status);
			matcher = regexStatus.matcher(line);
			matcher.find();
			response.status = Integer.parseInt(matcher.group(1));
