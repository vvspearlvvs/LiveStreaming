...
class RtcpDeinterleaver extends InputStream implements Runnable {
	...
	public void run() {
		try {
			while (true) {
				...
				Log.i(TAG,"read해서 인코딩한 데이터를 파이프로 OUTPUTSTREAM"+mPipedOutputStream);
			}
		} catch (IOException e) {
			try {
	...
	public int read(byte[] buffer) throws IOException {
		...
		Log.i(TAG,"RTCP로 전송할 버퍼 읽음"+buffer);
	...
	public int read(byte[] buffer, int offset, int length) throws IOException {
		...
		Log.i(TAG,"RTCP로 전송할 버퍼 자세한 내용"+offset+"길이"+length);
	...
	public int read() throws IOException {
		...
		Log.i(TAG,"실제로 인풋 파이프 라인에 들어있는거 읽ㅇ름");
