...
package com.example.wearable.camera6.Encoding;
...
public class EncoderDebugger {
	...
	private void debug() {
		...
		for (int i=0;i<encoders.length;i++) {
			for (int j=0;j<encoders[i].formats.length;j++) {
				...
				Log.i(TAG, ">> Test "+(n++)+"/"+count+": "+mEncoderName+" with color format "+mEncoderColorFormat+" at "+mWidth+"x"+mHeight);
				...
				try {
					...
					saveTestResult(true);
					Log.i(TAG, "The encoder "+mEncoderName+" is usable with resolution "+mWidth+"x"+mHeight);
					return;
	...
	private void configureDecoder() throws IOException {
		...
		if (decInputIndex>=0) {
			...
			if (VERBOSE) Log.e(TAG,"디코더 No buffer available !");
		}
			...
			if (decOutputIndex == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
				...
				if (n>2) {
					...
					if (j>=NB_DECODED-1) {
						...
						Log.i(TAG, "Decoding "+n+" frames took "+elapsed/1000+" ms");
						return elapsed;
					}
					j++;
				}
